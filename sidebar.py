""" sidebar.py — Barra lateral. Retorna um dicionário de configurações usadas pelas abas. """
import os
...
def render_sidebar() -> dict:
    """
    Renderiza a barra lateral e retorna os valores de configuração.
    ...
    with st.sidebar:
        ...
        role_label = "Administrador" if user_role == 'admin' else "Usuário"
        st.markdown(
        ...
        if st.button("Atualizar Câmbio", help="Buscar taxa atual USD/BRL", use_container_width=True):
            new_rate = get_live_fx()
            if new_rate:
                st.session_state['_fx_new_rate'] = new_rate
                st.toast(f"Câmbio atualizado: R$ {new_rate:.4f}")
            else:
                st.toast("Serviço indisponível. Edite manualmente.")
            st.rerun()
